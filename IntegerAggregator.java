...
public class IntegerAggregator implements Aggregator {
    ...
    private int afield;
    private Op what;
    private HashMap<Field, ArrayList<Integer>> group_agg_map;
    ...
    public void mergeTupleIntoGroup(Tuple tup) {
        ...
        {
            group_agg_map.put(tupgbField, new ArrayList<Integer>());
        }
        group_agg_map.get(tupgbField).add(new Integer(((IntField) tup.getField(this.afield)).getValue()));
    }

    // compute aggregate value from stored integer list depending on what the given operator is
    private Integer computeAgg(ArrayList<Integer> aggs) throws Exception {
        if (aggs.size() == 0) {
            return null;
        }
        if (this.what == COUNT) {
            return aggs.size();
        } else if (this.what == SUM) {
            Integer sum = 0;
            for (Integer agg : aggs) {
                sum += agg;
            }
            return sum;
        } else if (this.what == AVG) {
            Integer sum = 0;
            for (Integer agg : aggs) {
                sum += agg;
            }
            return sum / aggs.size();
        } else if (this.what == MIN) {
            return Collections.min(aggs);
        } else if (this.what == MAX) {
            return Collections.max(aggs);
        } else {
    ...
    public OpIterator iterator() {
        ...
        for (Map.Entry<Field, ArrayList<Integer>> entry : group_agg_map.entrySet()) {
            Integer agg = null;
            try {
